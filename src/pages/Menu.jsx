    ...
    <section className="section-padding">
      <Container>
        <div className="menu-filters" data-aos="fade-up">
          {categories.map(category => <button key={category} className={activeCategory === category ? 'active' : ''} onClick={() => setActiveCategory(category)}>{category}</button>)}</div>
        <Row className="g-4">
          {filtered.map(item => <Col md={6} lg={4} xl={3} key={item.id}>
            <MenuCard item={item} />
          </Col>)}
        </Row>
      </Container>
